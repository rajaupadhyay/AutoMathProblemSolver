'''
First iteration of the CNN model to predict the operation required for a question
Accuracy: 31%
'''
import pandas as pd
import random
...
def simple_CNN(X_train, y_train, X_test, y_test, qstn=None):
    # Obtain the vocab from the training dataset
    ...
    # the model will take as input an integer matrix of size (batch (32), input_length).
    # now model.output_shape == (None, input_length, output_dim = embedding_dim), where None is the batch dimension.
    model.add(layers.Conv1D(64, 5, padding='valid', kernel_initializer='normal', activation='relu'))
    # Use Conv1D to operate on the first dimension of the data only (Used for text mostly)
    # Convolution window of size 5 and output 64 filters in conv

    # Pool over the entire output rather than windows of the output
    model.add(layers.GlobalMaxPooling1D())
    # Dense is just a layer of neurons in a NN
